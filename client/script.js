...
const ohMy = () => {
  axios
    .get(`${localhost}/animals`)
    .then((res) => {
      console.log(res.data);
      //   console.log(res);
      for (let i = 0; i < res.data.length; i++) {
        const animalText = document.createElement("p");
        animalText.textContent = `${res.data[i]}`;
        document.querySelector("body").appendChild(animalText);
      }
    })
...
const repeatMyParam = (event) => {
  //YOUR CODE HERE
  axios.get(`${localhost}/repeat/${event.target.id}`).then((res) => {
    // console.log(res.data);
...
const queryRequest = () => {
  axios
    .get(`${localhost}/query-test?name=giselle&type=hello`)
    .then((res) => {
      console.log(res.data);
    })
    .catch((error) => console.log(error));
...
/*
    ...
    1: Send no queries on the URL -- what happened? sends back string saying no query entered

    2: Send more than 1 query on the URL -- what happened? if different keys, thenm sends nodelist of queries asked with string saying more than one query sent
*/
